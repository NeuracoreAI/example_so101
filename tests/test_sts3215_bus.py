...
from common.sts3215_bus import (
    SO101LeaderDriver,
    _decode_sign_magnitude,
    _normalize_degrees,
    _normalize_range_0_100,
)
...
class TestNormalizeRange0100:
    def test_min_is_zero(self) -> None:
        assert _normalize_range_0_100(
            val=1968, range_min=1968, range_max=3526
        ) == pytest.approx(0.0)

    def test_max_is_hundred(self) -> None:
        assert _normalize_range_0_100(
            val=3526, range_min=1968, range_max=3526
        ) == pytest.approx(100.0)
...
class TestSO101LeaderDriver:
    def _make_driver(
        self, tmp_path: Path, raw_values: dict[int, int]
    ) -> SO101LeaderDriver:
    ...
    def test_connect_opens_port(self, tmp_path: Path) -> None:
        driver = self._make_driver(tmp_path, {})
        driver.connect()
        driver._port_handler.openPort.assert_called_once()
        assert driver.is_connected is True

    def test_disconnect_closes_port(self, tmp_path: Path) -> None:
        driver = self._make_driver(tmp_path, {})
        driver.connect()
        driver.disconnect()
        driver._port_handler.closePort.assert_called_once()

    def test_get_action_returns_six_keys(self, tmp_path: Path) -> None:
        raw = {
    ...
    def test_get_action_body_joint_near_zero_at_mid(self, tmp_path: Path) -> None:
    ...
    def test_get_action_gripper_near_fifty_at_mid(self, tmp_path: Path) -> None:
